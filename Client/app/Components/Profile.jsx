...
const Profile = ({setShowProfile}) => {
  const { user, changeProfilePhoto } = useContext(AuthContext)
    ...
    <div className='flex absolute bg-black border-[1px] border-yellow-600 rounded-md top-1/2 left-1/2 translate-x-[-50%] translate-y-[-50%] items-center flex-col gap-3 w-[80%] md:w-[400px] p-5'> 
      <div className='flex items-center flex-col gap-2'>
        ...
        <label htmlFor="file" className='w-full p-3'>
          <Image  src={image ? URL.createObjectURL(image) : user?.profilePhoto?.url} alt="profile photo" width={200} height={200} className={"w-[60px] h-[60px] rounded-full"}/>
        </label>
        <button onClick={()=> changeProfilePhoto(image , user._id)} className='border-[1px] border-yellow-600 p-1 w-[70px] rounded-sm text-yellow-600'>Update</button>
      </div>
